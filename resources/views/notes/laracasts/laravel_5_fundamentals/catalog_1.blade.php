<table class="table">
  <thead>
    <th>序号</th>
    <th>标题</th>
    <th>笔记</th>
  </thead>
  <tbody>
    <tr>
      <td>01</td>
      <td>
        <a href="/notes/laracasts/laravel_5_fundamentals/01_meet_composer">
          认识 composer
        </a>
      </td>
      <td><span class="label label-danger">更新</span></td>
    </tr>
    <tr>
      <td>02</td>
      <td>
        <a href="/notes/laracasts/laravel_5_fundamentals/02_virtual_machines_and_homestead">
          认识虚拟机和 Homestead
        </a>
      </td>
      <td><span class="label label-warning">简介</span></td>
    </tr>
    <tr>
      <td>03</td>
      <td>
        <a href="/notes/laracasts/laravel_5_fundamentals/03_a_gentle_introduction_to_routing_controllers_and_Views">
          路由、控制器和视图简介
        </a>
      </td>
      <td><span class="label label-warning">简介</span></td>
    </tr>
    <tr>
      <td>04</td>
      <td>
        <a href="/notes/laracasts/laravel_5_fundamentals/04_passing_data_to_views">
          给视图传递数据
        </a>
      </td>
      <td><span class="label label-default">无</span></td>
    </tr>
    <tr>
      <td>05</td>
      <td>
        <a href="/notes/laracasts/laravel_5_fundamentals/05_blade_101">
          Blade视图模板基础
        </a>
      </td>
      <td><span class="label label-default">无</span></td>
    </tr>
    <tr>
      <td>06</td>
      <td>
        <a href="/notes/laracasts/laravel_5_fundamentals/06_environments_and_configuration">
          环境和配置
        </a>
      </td>
      <td><span class="label label-default">无</span></td>
    </tr>
    <tr>
      <td>07</td>
      <td>
        <a href="/notes/laracasts/laravel_5_fundamentals/07_migrations">
          数据库迁移
        </a>
      </td>
      <td><span class="label label-default">无</span></td>
    </tr>
    <tr>
      <td>08</td>
      <td>
        <a href="/notes/laracasts/laravel_5_fundamentals/08_Eloquent_101">
          Eloquent 基础
        </a>
      </td>
      <td><span class="label label-default">无</span></td>
    </tr>
    <tr>
      <td>09</td>
      <td>
        <a href="/notes/laracasts/laravel_5_fundamentals/09_basic_model_controller_view_workflow">
          MVC 基础工作流
        </a>
      </td>
      <td><span class="label label-default">无</span></td>
    </tr>
    <tr>
      <td>10</td>
      <td>
        <a href="/notes/laracasts/laravel_5_fundamentals/10_forms">
          表单
        </a>
      </td>
      <td><span class="label label-default">无</span></td>
    </tr>
  </tbody>
</table>
